...
Morebits.i18n = {
	...
	setParser: function(parser) {
		if (!parser || typeof parser.get !== 'function') {
			throw new Error('Morebits.i18n: parser harus mengimplementasikan get()');
		}
...
Morebits.quickForm.element.prototype.compute = function QuickFormElementCompute(data, in_id) {
	...
	switch (data.type) {
		...
		case 'textarea':
			...
			break;
		default:
			throw new Error('Morebits.quickForm: jenis elemen tidak diketahui ' + data.type.toString());
	}
...
Morebits.string = {
	...
	splitWeightedByKeys: function(str, start, end, skiplist) {
		if (start.length !== end.length) {
			throw new Error('panjang penanda awal dan penanda akhir harus sama');
		}
		let level = 0;
		let initial = null;
		const result = [];
		if (!Array.isArray(skiplist)) {
			if (skiplist === undefined) {
				...
				throw new Error('paramater daftar lewati yang tidak ter-aplikasi');
			}
		}
...
Morebits.unbinder = function Unbinder(string) {
	if (typeof string !== 'string') {
		throw new Error('bukan sebuah string');
	}
...
Morebits.unbinder.prototype = {
	...
	unbind: function UnbinderUnbind(prefix, postfix) {
		if (!prefix || !postfix) {
			throw new Error('Kedua prefix dan postfix harus disediakan');
		}
...
Morebits.date.localeData = {
	...
	months: [msg('january', 'Januari'), msg('february', 'Februari'), msg('march', 'Maret'),
		msg('april', 'April'), msg('may_long', 'Mei'), msg('june', 'Juni'),
		msg('july', 'Juli'), msg('august', 'Agustus'), msg('september', 'September'),
		msg('october', 'Oktober'), msg('november', 'November'), msg('december', 'Desember')],
	monthsShort: [msg('jan', 'Jan'), msg('feb', 'Feb'), msg('mar', 'Mar'),
		msg('apr', 'Apr'), msg('may', 'Mei'), msg('jun', 'Jun'),
		msg('jul', 'Jul'), msg('aug', 'Agu'), msg('sep', 'Sep'),
		msg('oct', 'Okt'), msg('nov', 'Nov'), msg('dec', 'Des')],
	days: [msg('sunday', 'Minggu'), msg('monday', 'Senin'), msg('tuesday', 'Selasa'),
		msg('wednesday', 'Rabu'), msg('thursday', 'Kamis'), msg('friday', 'Jumat'),
		msg('saturday', 'Sabtu')],
	daysShort: [msg('sun', 'Ming'), msg('mon', 'Sen'), msg('tue', 'Sel'),
		msg('wed', 'Rab'), msg('thu', 'Kam'), msg('fri', 'Jum'),
		msg('sat', 'Sab')],

	relativeTimes: {
		thisDay: msg('relative-today', '[Hari ini pada] h:mm A'),
		prevDay: msg('relative-prevday', '[Kemarin pada] h:mm A'),
...
Morebits.date.prototype = {
	...
	add: function(number, unit) {
		...
		if (isNaN(num)) {
			throw new Error('Angka"' + number + '"yang disediakan tidak valid.');
		}
	...
	format: function(formatstr, zone) {
		if (!this.isValid()) {
			return 'Tanggal tidak valid'; // Put the truth out, preferable to "NaNNaNNan NaN:NaN" or whatever
		}
		let udate = this;
...
Morebits.wiki.api.getToken = function() {
	const tokenApi = new Morebits.wiki.api(msg('getting-token', 'Memperoleh token'), {
		action: 'query',
		meta: 'tokens',
		type: 'csrf',
		format: 'json'
...
Morebits.wiki.page = function(pageName, status) {
	...
	if (!status) {
		status = msg('opening-page', pageName, 'Membuka halaman "' + pageName + '"');
	}
	...
	this.load = function(onSuccess, onFailure) {
		...
		ctx.loadApi = new Morebits.wiki.api(msg('retrieving-page', 'Mengambil halaman...'), ctx.loadQuery, fnLoadSuccess, ctx.statusElement, ctx.onLoadFailure);
	...
	this.save = function(onSuccess, onFailure) {
		...
		if (!ctx.pageLoaded && !canUseMwUserToken) {
			ctx.statusElement.error('Kesalahan internal: mencoba untuk menyimpan sebuah halaman yang belum dimuat!');
		...
		if (ctx.fullyProtected && !ctx.suppressProtectWarning &&
			...
			ctx.statusElement.error(msg('protected-aborted', 'Suntingan pada halaman perlindungan penuh dibatalkan.'));
			ctx.onSaveFailure(this);
			return;
		}
		...
		switch (ctx.editMode) {
			...
			case 'new':
				if (!ctx.newSectionText) { // API doesn't allow empty new section text
					ctx.statusElement.error('Kesalahan internal: teks bagian baru tidak ditentukan sebelum disimpan!');
					ctx.onSaveFailure(this);
					return;
				}
				query.section = 'new';
		...
		ctx.saveApi = new Morebits.wiki.api(msg('saving-page', 'Menyimpan halaman...'), query, fnSaveSuccess, ctx.statusElement, fnSaveError);
	...
	this.setFollowRedirect = function(followRedirect, followCrossNsRedirect) {
		if (ctx.pageLoaded) {
			ctx.statusElement.error('Kesalahan internal: tidak dapat mengubah pengaturan pengalihan setelah halaman dimuat!');
			return;
		}
	...
	this.lookupCreation = function(onSuccess, onFailure) {
		...
		ctx.lookupCreationApi = new Morebits.wiki.api(msg('getting-creator', 'Memperoleh informasi pembuatan halaman'), query, fnLookupCreationSuccess, ctx.statusElement, ctx.onLookupCreationFailure);
	...
	this.revert = function(onSuccess, onFailure) {
		...
		if (!ctx.revertOldID) {
			ctx.statusElement.error('Kesalahan internal: ID revisi tidak ditentukan sebelum dikembalikan!');
			ctx.onSaveFailure(this);
			return;
		}
	...
	this.move = function(onSuccess, onFailure) {
		...
		if (!ctx.moveDestination) {
			ctx.statusElement.error('Kesalahan internal: nama halaman tujuan belum di atur sebelum dipindahkan!');
		...
		if (fnCanUseMwUserToken('move')) {
			...
			ctx.moveApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), query, fnProcessMove, ctx.statusElement, ctx.onMoveFailure);
	...
	this.patrol = function() {
		...
		if ($('.patrollink').length) {
			...
			ctx.patrolApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), patrolQuery, fnProcessPatrol);
	...
	this.triage = function() {
		...
		if (!mw.config.get('pageTriageNamespaces').includes(new mw.Title(ctx.pageName).getNamespaceId())) {
			...
			if (new mw.Title(Morebits.pageNameNorm).getPrefixedText() === new mw.Title(ctx.pageName).getPrefixedText()) {
				...
				ctx.triageApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), query, fnProcessTriageList);
	...
	this.deletePage = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('delete')) {
			...
			ctx.deleteApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), query, fnProcessDelete, ctx.statusElement, ctx.onDeleteFailure);
	...
	this.undeletePage = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('undelete')) {
			...
			ctx.undeleteApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), query, fnProcessUndelete, ctx.statusElement, ctx.onUndeleteFailure);
	...
	this.protect = function(onSuccess, onFailure) {
		...
		ctx.protectApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), query, fnProcessProtect, ctx.statusElement, ctx.onProtectFailure);
	...
	this.stabilize = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('stabilize')) {
			...
			ctx.stabilizeApi = new Morebits.wiki.api(msg('getting-token', 'memperoleh token...'), query, fnProcessStabilize, ctx.statusElement, ctx.onStabilizeFailure);
	...
	var fnLoadSuccess = function() {
		...
		if (!ctx.csrfToken) {
			ctx.statusElement.error(msg('token-fetch-fail', 'Gagal untuk memperoleh token penyuntingan.'));
			ctx.onLoadFailure(this);
			return;
		}
		ctx.loadTime = ctx.loadApi.getResponse().curtimestamp;
		if (!ctx.loadTime) {
			ctx.statusElement.error('Gagal untuk menerima stempel waktu sekarang.');
			ctx.onLoadFailure(this);
			return;
		}
		...
		if (ctx.editMode === 'revert') {
			...
			if (!ctx.revertCurID) {
				ctx.statusElement.error('Gagal untuk menerima ID revisi sekarang.');
				ctx.onLoadFailure(this);
				return;
			}
			...
			if (!ctx.revertUser) {
				if (rev && rev.userhidden) { // username was RevDel'd or oversighted
					...
					ctx.statusElement.error('Gagal untuk memperoleh pengguna yang membuat revisinya.');
					ctx.onLoadFailure(this);
					return;
				}
			}
	...
	var fnCheckPageName = function(response, onFailure) {
		...
		if (page) {
			...
			if (page.invalid) {
				ctx.statusElement.error(msg('invalid-title', ctx.pageName, 'Judul halaman tidak valid: ' + ctx.pageName));
				onFailure(this);
				return false; // abort
			}
			...
			ctx.statusElement.error(msg('redirect-resolution-fail', ctx.pageName, 'Tidak dapat menyelesaikan pengalihan untuk: ' + ctx.pageName));
			onFailure(this);
	...
	var fnSaveError = function() {
		...
		if (errorCode === 'editconflict' && ctx.conflictRetries++ < ctx.maxConflictRetries) {
			...
			const purgeApi = new Morebits.wiki.api(msg('editconflict-purging', 'Konflik penyuntingan terdeteksi, menghapus tembolok peladen'), purgeQuery, (() => {
				--Morebits.wiki.numberOfActionsLeft; // allow for normal completion if retry succeeds

				ctx.statusElement.info(msg('editconflict-retrying', 'Konflik penyuntingan terdeteksi, menambahkan kembali suntingan'));
				if (fnCanUseMwUserToken('edit')) {
		...
		// hard error, give up
			...
			switch (errorCode) {
				...
				case 'protectedpage':
					...
					ctx.statusElement.error('Gagal untuk menyimpan suntingan: halaman dilindungi');
					break;
				...
				case 'abusefilter-warning':
					ctx.statusElement.error([ 'Sebuah peringatan telah dikembalikan oleh filter penyuntingan: "', errorData.abusefilter.description, '". Jika anda ingin melanjutkan suntingan, tolong coba ulang. Peringatan ini tidak akan muncul dua kali.' ]);
				...
				case 'spamblacklist':
					...
					ctx.statusElement.error('Tidak dapat menyimpan halaman dikarenakan URL ' + spam + ' dalam daftar hitam spam');
					break;

				default:
					ctx.statusElement.error('Gagal untuk menyimpan halaman: ' + ctx.saveApi.getErrorText());
			}
	...
	var fnLookupCreationSuccess = function() {
		...
		if (!rev) {
			ctx.statusElement.error('Tidak dapat menemukan revisi dari ' + ctx.pageName);
			ctx.onLookupCreationFailure(this);
			return;
		}
		...
		if (!ctx.lookupNonRedirectCreator || !isTextRedirect(rev.content)) {
			...
			if (!ctx.creator) {
				ctx.statusElement.error('Tidak dapat menemukan nama pembuat halaman');
				ctx.onLookupCreationFailure(this);
				return;
			}
			ctx.timestamp = rev.timestamp;
			if (!ctx.timestamp) {
				ctx.statusElement.error('Tidak dapat menemukan stempel waktu pembuatan halaman');
				ctx.onLookupCreationFailure(this);
				return;
			}

			ctx.statusElement.info('menerima informasi pembuatan halaman');
			...
			ctx.lookupCreationApi = new Morebits.wiki.api('Menerima informasi pembuatan halaman', ctx.lookupCreationApi.query, fnLookupNonRedirectCreator, ctx.statusElement, ctx.onLookupCreationFailure);
			ctx.lookupCreationApi.setParent(this);
			ctx.lookupCreationApi.post();
		}
	...
	var fnLookupNonRedirectCreator = function() {
		...
		if (!ctx.timestamp) {
			ctx.statusElement.error('Tidak dapat menemukan stempel waktu dari pembuatan halaman');
			ctx.onLookupCreationFailure(this);
			return;
		}
	...
	var fnPreflightChecks = function(action, onFailure) {
		...
		if (!Morebits.userIsSysop && action !== 'move') {
			ctx.statusElement.error('Tidak dapat ' + action + 'halaman : hanya pengurus dapat melakukan');
			onFailure(this);
			return false;
		}

		if (!ctx.editSummary) {
			ctx.statusElement.error('Kesalahan internal: ' + action + ' belum mengatur alasan (gunakan fungsi setEditSummary)!');
			onFailure(this);
			return false;
		}
		return true; // all OK
	...
	const fnProcessChecks = function(action, onFailure, response) {
		...
		if (!response.tokens.csrftoken) {
			ctx.statusElement.error('Gagal mendapatkan token.');
			onFailure(this);
			return false;
		}
		return true; // all OK
	...
	var fnProcessMove = function() {
		...
		ctx.moveProcessApi = new Morebits.wiki.api(msg('moving-page', 'memindahkan halaman...'), query, ctx.onMoveSuccess, ctx.statusElement, ctx.onMoveFailure);
	...
	var fnProcessPatrol = function() {
		...
		const patrolStat = new Morebits.status('Menandai halaman sebagai terpatroli');

		ctx.patrolProcessApi = new Morebits.wiki.api('mematroli halaman...', query, null, patrolStat);
	...
	var fnProcessDelete = function() {
		...
		ctx.deleteProcessApi = new Morebits.wiki.api('menghapus halaman...', query, ctx.onDeleteSuccess, ctx.statusElement, fnProcessDeleteError);
	...
	var fnProcessDeleteError = function() {
		...
		if (errorCode === 'internal_api_error_DBQueryError' && ctx.retries++ < ctx.maxRetries) {
			...
			ctx.statusElement.error('Tidak dapat menghapus halaman karena sudah tidak ada');
			if (ctx.onDeleteFailure) {
				ctx.onDeleteFailure.call(this, ctx.deleteProcessApi); // invoke callback
			}
		// hard error, give up
		} else {
			ctx.statusElement.error('Gagal menghapus halaman: ' + ctx.deleteProcessApi.getErrorText());
			if (ctx.onDeleteFailure) {
				ctx.onDeleteFailure.call(this, ctx.deleteProcessApi); // invoke callback
			}
		}
	...
	var fnProcessUndelete = function() {
		...
		ctx.undeleteProcessApi = new Morebits.wiki.api('memulihkan halaman...', query, ctx.onUndeleteSuccess, ctx.statusElement, fnProcessUndeleteError);
	...
	var fnProcessUndeleteError = function() {
		...
		if (errorCode === 'internal_api_error_DBQueryError') {
			if (ctx.retries++ < ctx.maxRetries) {
				ctx.statusElement.info('Galat peyimpanan kueri, mecoba ulang');
				--Morebits.wiki.numberOfActionsLeft; // allow for normal completion if retry succeeds
			...
			ctx.statusElement.error('Tidak dapat memulihkan halaman, dikarenakan tidak ada revisi untuk dipulihkan atau sudah dipulihkan');
			if (ctx.onUndeleteFailure) {
				ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi); // invoke callback
			}
		// hard error, give up
		} else {
			ctx.statusElement.error('Gagal untuk memulihkan halaman: ' + ctx.undeleteProcessApi.getErrorText());
			if (ctx.onUndeleteFailure) {
				ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi); // invoke callback
			}
		}
	...
	var fnProcessProtect = function() {
		...
		ctx.protectProcessApi = new Morebits.wiki.api('melindungi halaman...', query, ctx.onProtectSuccess, ctx.statusElement, ctx.onProtectFailure);
	...
	var fnProcessStabilize = function() {
		...
		*/
		...
		ctx.stabilizeProcessApi = new Morebits.wiki.api('mengonfigurasi pengaturan stabilisasi...', query, ctx.onStabilizeSuccess, ctx.statusElement, ctx.onStabilizeFailure);
...
Morebits.wiki.preview = function(previewbox) {
	...
	this.beginRender = function(wikitext, pageTitle, sectionTitle) {
		...
		const renderApi = new Morebits.wiki.api('memuat...', query, fnRenderSuccess, new Morebits.status('Pratinjau'));
	...
	var fnRenderSuccess = function(apiobj) {
		...
		if (!html) {
			apiobj.statelem.error('Gagal untuk mengambil pratinjau, atau templat kosong');
			return;
		}
...
Morebits.userspaceLogger = function(logPageName) {
	if (!logPageName) {
		throw new Error('tidak ada nama catatan halaman dispesifikan');
	}
	/**
	...
	this.log = function(logText, summaryText) {
		...
		const page = new Morebits.wiki.page('Pengguna:' + mw.config.get('wgUserName') + '/' + logPageName,
			'Menambahkan entri ke log ruangnama'); // make this '... to ' + logPageName ?
		page.load((pageobj) => {
...
Morebits.status.init = function(root) {
	if (!(root instanceof Element)) {
		throw new Error('objek bukan merupakan sebuah bagian dari Element');
	}
...
Morebits.status.onError = function(handler) {
	if (typeof handler === 'function') {
		...
		throw new Error('Morebits.status.onError: handler bukanlah sebuah fungsi');
	}
};
...
Morebits.batchOperation = function(currentAction) {
	const ctx = {
		...
		statusElement: new Morebits.status(currentAction || msg('batch-starting', 'Melakukan operasi tindakan')),
	...
	this.run = function(worker, postFinish) {
		...
		if (!total) {
			ctx.statusElement.info(msg('batch-no-pages', 'tidak ada halaman disebutkan'));
			ctx.running = false;
			if (ctx.postFinish) {
				ctx.postFinish();
			}
	...
	this.workerSuccess = function(arg) {
		...
		if (arg instanceof Morebits.wiki.api || arg instanceof Morebits.wiki.page) {
			...
			new Morebits.status(arg, msg('batch-done-page', arg, 'selesai ([[' + arg + ']])'));
		}
	...
	var fnDoneOne = function() {
		...
		if (ctx.countFinished < total) {
			...
			ctx.statusElement.status(msg('persen', progress, progress + '%'));
			...
			const statusString = msg('batch-progress', ctx.countFinishedSuccess, ctx.countFinished, 'Selesai (' + ctx.countFinishedSuccess +
				'/' + ctx.countFinished + ' tindakan diselesaikan dengan baik)');
...
Morebits.simpleWindow = function SimpleWindow(width, height) {
	...
	$(this.content).dialog({
		autoOpen: false,
		buttons: { 'Tombol placeholder': function() {} },
